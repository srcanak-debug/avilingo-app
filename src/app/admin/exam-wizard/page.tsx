...
function ExamWizardInner() {
  ...
  async function handleSave() {
    ...
    // Create exams for selected candidates
    if (data.selected_candidates.length > 0 && templateId) {
    ...
    await supabase.from('audit_logs').insert({
      ...
      details: { name: data.name, candidates_assigned: data.selected_candidates.length }
    })
    ...
    <div style={{minHeight:'100vh',background:'var(--off)',fontFamily:'var(--fb)'}}>
      ...
      <div style={{display:'flex',maxWidth:'1200px',margin:'0 auto',padding:'24px 28px',gap:'24px'}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          {step === 7 && (
            <div style={card()}>
              <h2 style={{fontFamily:'var(--fm)',fontSize:'18px',fontWeight:800,color:'var(--navy)',marginBottom:'4px'}}>Assign Candidates</h2>
              <p style={{fontSize:'13px',color:'var(--t3)',marginBottom:'6px'}}>Select candidates to assign this exam. You can skip this and assign later.</p>
              ...
              {data.selected_candidates.length > 0 && (
                <div style={{padding:'10px 14px',borderRadius:'8px',background:'#EAF3DE',marginBottom:'14px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  <span style={{fontSize:'13px',fontWeight:600,color:'#27500A'}}>{data.selected_candidates.length} candidate{data.selected_candidates.length > 1 ? 's' : ''} selected</span>
                  <button onClick={()=>upd({selected_candidates:[]})} style={{padding:'3px 10px',borderRadius:'5px',border:'1.5px solid #BBF7D0',background:'#fff',cursor:'pointer',fontSize:'11px',fontWeight:600,color:'#27500A',fontFamily:'var(--fb)'}}>Clear all</button>
                </div>
              )}
              ...
              <div style={{maxHeight:'340px',overflowY:'auto',border:'1px solid var(--bdr)',borderRadius:'10px'}}>
                {filteredCandidates.length === 0 ? (
                  <div style={{padding:'32px',textAlign:'center',color:'var(--t3)',fontSize:'13px'}}>
                    {candidates.length === 0 ? 'No candidates in the system yet.' : 'No candidates match your search.'}
                  </div>
                ) : (
                  filteredCandidates.map((c, i) => {
                    ...
                    return (
                      <div key={c.id} onClick={()=>{
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:'13px',fontWeight:600,color:'var(--navy)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.full_name || 'Unnamed'}</div>
                          <div style={{fontSize:'11.5px',color:'var(--t3)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.email}</div>
                        </div>
                        <div style={{fontSize:'10.5px',color:'var(--t3)',flexShrink:0}}>{c.organizations?.name || '—'}</div>
                      </div>
                    )
                  })
                )}
              ...
              {data.selected_candidates.length > 0 && (
                <div style={{marginTop:'18px'}}>
                  <label style={label}>Invitation Message (optional)</label>
                  <textarea value={data.invite_message} onChange={e=>upd({invite_message:e.target.value})}
                    rows={3} style={{...inp(),resize:'vertical'}} />
                  <div style={{fontSize:'11px',color:'var(--t3)',marginTop:'4px'}}>
                    This message will be included in the email invitation sent to selected candidates.
                  </div>
                </div>
              )}
            </div>
