...
// TMS Components
const QualificationTracker = dynamic(
  () => import("./components/tms/QualificationTracker"),
);
const ResourceManager = dynamic(
  () => import("./components/tms/ResourceManager"),
);
...
const CategoryManager = dynamic(
  () => import("./components/lms/CategoryManager"),
);

// EBT Components
const EbtMatrix = dynamic(() => import("./components/ebt/EbtMatrix"));
const AssessmentForms = dynamic(
  () => import("./components/ebt/AssessmentForms"),
);

// CompBT Components
const AdaptiveLearning = dynamic(
  () => import("./components/compbt/AdaptiveLearning"),
);
...
// AI Components
const ContentTransformer = dynamic(
  () => import("./components/ai/ContentTransformer"),
);
const QuestionGenerator = dynamic(
  () => import("./components/ai/QuestionGenerator"),
);

// Virtual & VR Components
const LiveClassroom = dynamic(
  () => import("./components/virtual/LiveClassroom"),
);
...
const COMPETENCY_TAGS: Record<string, string[]> = {
  ...
  reading: [
    "sop_comprehension",
    "notam_interpretation",
    "weather_report_reading",
    "technical_manual",
    "safety_card_reading",
    "atc_clearance_reading",
  ],
  listening: [
    "atc_phraseology",
    "cockpit_communication",
    "cabin_announcement",
    "emergency_broadcast",
    "ground_ops_radio",
...
const DEFAULT_PREP = {
  grammar: {
    seconds: 45,
    bullets: [
      "The grammar section includes multiple-choice questions.",
      "Read each question carefully and select the best answer.",
      "There are no penalties for wrong answers.",
      "Your movements, screen activity, and eye activity are monitored.",
    ],
  },
  reading: {
    seconds: 45,
    bullets: [
      "The reading section includes texts followed by multiple-choice questions.",
      "Each question has only one correct answer. Read the texts carefully.",
      "There are no penalties for incorrect answers.",
      "Your movements, screen activity, and eye activity are being monitored.",
    ],
  },
  writing: {
    seconds: 45,
    bullets: [
      "Write at least 40 words for each question. Do not copy or paste from any source.",
      "If time runs out, the system will automatically save your response.",
      "Your movements, eye activity, and screen are being monitored.",
    ],
  },
  speaking: {
    seconds: 45,
    bullets: [
      "You must speak for at least 30 seconds to meet the minimum requirement.",
      "Do not use written texts, memorized speeches, ChatGPT, or any other AI tools.",
      "When you are ready, press the record button to start.",
    ],
  },
  listening: {
    seconds: 45,
    bullets: [
      "Each audio will be played once only.",
      "There are no penalties for wrong answers, so try to answer every question.",
      "When you are ready, press the Play button to begin listening.",
    ],
  },
};
...
export default function AdminDashboard() {
  ...
  async function loadStats() {
    ...
    const { data: pendingGrading } = await supabase
      .from("exam_answers")
      .select("id, exams!inner(candidate_id, role_profile)")
      .eq("exams.status", "grading")
      .limit(5);
  ...
  async function runQuery(page = qPage, overrides: any = {}) {
    ...
    let query = supabase
      .from("questions")
      .select(
        "*,question_analytics(difficulty_index,total_attempts,last_used_at),question_assignments(id,departments(name),sub_roles(name))",
        { count: "exact" },
      )
      .eq("is_latest", true)
    ...
    if (sort === "newest")
      ...
      query = query.order("cefr_level", { ascending: true });
    ...
    if (editQ) {
      ...
      if ((count || 0) > 0) {
        ...
        const { data: newQ } = await supabase
          ...
          .insert({
            ...payload,
            version_number: (editQ.version_number || 1) + 1,
            parent_question_id: editQ.parent_question_id || editQ.id,
            is_latest: true,
          })
          .select()
          .single();
      ...
      const { data: newQ } = await supabase
        .from("questions")
        .insert({ ...payload, version_number: 1, is_latest: true })
        .select()
        .single();
      qId = newQ?.id;
      if (qId)
        await supabase
          ...
          .insert({ question_id: qId, total_attempts: 0, correct_count: 0 });
    }
  ...
  async function saveOptionsAndRubrics(qId: string) {
    ...
    if (validRubrics.length)
      await supabase
        .from("question_rubrics")
        .insert(
          validRubrics.map((r, i) => ({
            question_id: qId,
    ...
    if (selectedSubRoles.length) {
      selectedSubRoles.forEach((sId) => {
        if (selectedUseCases.length)
          selectedUseCases.forEach((uId) =>
            assignments.push({
              question_id: qId,
              sub_role_id: sId,
              use_case_id: uId,
              min_cefr: formQ.cefr_level,
            }),
          );
        else
          assignments.push({
  ...
  function startEdit(q: any) {
    ...
    setFormQ({
      ...
      cefr_level: q.cefr_level || "B1",
      difficulty: q.difficulty || "medium",
      competency_tag: q.competency_tag || "",
      aircraft_context: q.aircraft_context || "",
      audio_url: q.audio_url || "",
      image_url: q.image_url || "",
      active: q.active,
      role_tag: q.role_tag || "general",
      dla_section: q.dla_section || "general",
      reading_text: q.reading_text || "",
      answer_time_sec: q.answer_time_sec || 75,
    });
    setOptions([
      { text: "", is_correct: false },
      { text: "", is_correct: false },
  ...
  async function exportQuestions() {
    ...
    const csv = [
      headers.join(","),
      ...data.map((q: any) => headers.map((h) => escape(q[h])).join(",")),
    ].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `questions-${qSection}-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
    ...
    for (const line of lines) {
      if (/^\d+[\.\)]\s/.test(line)) {
        ...
        current.correct_answer = line
          .replace(/^(Answer|Correct|Key|ANSWER)[\s:]*/i, "")
          .trim();
  ...
  async function handleFileUpload(file: File) {
    ...
    if (ext === "txt") {
      ...
      setBulkStatus(`Found ${parsed.length} questions.`);
    } else if (ext === "csv") {
  ...
  async function confirmBulkUpload() {
    ...
    for (let i = 0; i < bulkParsed.length; i += BATCH) {
      const { error } = await supabase
        .from("questions")
        .insert(bulkParsed.slice(i, i + BATCH));
      if (error)
        errs.push(`Batch ${Math.floor(i / BATCH) + 1}: ${error.message}`);
    }
    setBulkStatus(
      errs.length === 0
        ? `✅ Imported ${bulkParsed.length} questions!`
        : `⚠️ ${errs.length} errors.`,
    );
    setBulkParsed([]);
    setBulkLoading(false);
    runQuery(0);
  ...
  async function loadAIFile(file: File) {
    ...
    setAiQueue(
      rows.map((r) => ({
        ...r,
        ai_section: r.section || "",
        ai_cefr: r.cefr_level || "",
        ai_difficulty: r.difficulty || "medium",
        ai_tag: r.competency_tags?.split("|")[0] || "",
        reviewed: false,
      })),
    );
  }
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      if (!q.ai_cefr || !cefrs.includes(q.ai_cefr)) {
        ...
        if (
          ...
          q.ai_cefr = "B2";
        else if (text.includes("describe") || text.includes("what would you"))
          q.ai_cefr = "B1";
    ...
    for (let i = 0; i < aiQueue.length; i += BATCH) {
      const batch = aiQueue
        ...
        .map((q) => ({
          content: q.content,
          correct_answer: q.correct_answer || "",
          cefr_level: q.ai_cefr || "B1",
          difficulty: q.ai_difficulty || "medium",
          section: q.ai_section || "grammar",
          competency_tag: q.ai_tag || "",
          aircraft_context: q.aircraft_context || "",
          audio_url: q.audio_url || "",
          active: true,
          type: "multiple_choice",
  ...
  async function saveTemplate() {
    ...
    const total =
      newTemplate.weight_grammar +
      newTemplate.weight_reading +
      newTemplate.weight_writing +
      newTemplate.weight_speaking +
      newTemplate.weight_listening +
      (newTemplate as any).weight_dla;
    if (Math.abs(total - 100) > 0.1) {
      alert("Weights must add up to 100%. Current: " + total + "%");
    ...
    const payload = {
      ...
      dla_count: (newTemplate as any).dla_count,
      ...
      weight_dla: (newTemplate as any).weight_dla,
      ...
      prep_grammar: (newTemplate as any).prep_grammar,
      prep_reading: (newTemplate as any).prep_reading,
      prep_writing: (newTemplate as any).prep_writing,
      prep_speaking: (newTemplate as any).prep_speaking,
      prep_listening: (newTemplate as any).prep_listening,
    };
    if (editTemplate) {
      const { error } = await supabase
        .from("exam_templates")
    ...
    setNewTemplate({
      ...
      attempts_allowed: 1,
      org_id: null,
      prep_grammar: DEFAULT_PREP.grammar,
      prep_reading: DEFAULT_PREP.reading,
      prep_writing: DEFAULT_PREP.writing,
      prep_speaking: DEFAULT_PREP.speaking,
      prep_listening: DEFAULT_PREP.listening,
    } as any);
    loadTemplates();
  }
  ...
  function startEditTemplate(t: any) {
    ...
    setNewTemplate({
      name: t.name,
      role_profile: t.role_profile,
      grammar_count: t.grammar_count,
      reading_count: t.reading_count,
      writing_count: t.writing_count,
      speaking_count: t.speaking_count,
      listening_count: t.listening_count,
      dla_count: t.dla_count || 10,
      weight_grammar: t.weight_grammar,
      weight_reading: t.weight_reading,
      weight_writing: t.weight_writing,
      weight_speaking: t.weight_speaking,
      weight_listening: t.weight_listening,
      weight_dla: t.weight_dla || 10,
      time_limit_mins: t.time_limit_mins,
      writing_timer_mins: t.writing_timer_mins || 3.5,
      speaking_attempts: t.speaking_attempts || 3,
      listening_single_play: t.listening_single_play !== false,
      passing_cefr: t.passing_cefr,
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
                ...
                {group.items.map((item) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (item.id === "hr_analytics")
                        return router.push("/hr/analytics");
        ...
        >
          <div style={{ padding: "20px 24px", flex: 1, overflowY: "auto" }}>
            ...
            {activeSection === "active_exams" && (
              <ActiveExams adminId={adminId} />
            )}
            ...
            {activeSection === "organizations" && (
              <OrganizationList
                ...
                setOrgStep={setOrgStep}
                startEditOrg={startEditOrg}
                startSingleDeleteOrg={startSingleDeleteOrg}
              />
            )}
            ...
            >
              ...
              {delItems.length > 5 && (
                ...
                >
                  ...
                  >
                    Type "DELETE" to confirm bulk action
                  </label>
                  <input
                    value={delInput}
                    onChange={(e) => setDelInput(e.target.value.toUpperCase())}
                    placeholder="DELETE"
                    style={{
                      width: "100%",
                      padding: "9px 12px",
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button
                  onClick={finalDelete}
                  disabled={delItems.length > 5 && delInput !== "DELETE"}
                  style={{
                    flex: 1,
                    padding: "10px",
                    borderRadius: "8px",
                    ...
                    fontFamily: "var(--fb)",
                    opacity:
                      delItems.length > 5 && delInput !== "DELETE" ? 0.5 : 1,
                  }}
                >
        ...
        {(showForm ||
          ...
          <>
            ...
            >
                        ...
                        : showOrgForm
                          ...
                          : detailUser
                            ...
                            : detailOrg
                              ...
                              : showForm
                                ? editQ
                                  ? `Edit Question V${editQ.version_number}`
                                  : "New Question"
                                : "Spotlight Details"}
                  </h3>
                  {detailQ && (
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g. ICAO Cabin Crew Recruitment"
                        style={inp({ width: "100%" })}
                      />
                    </div>
                    ...
                    >
                      <div>
                        ...
                        <select
                          value={newTemplate.role_profile}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                              role_profile: e.target.value,
                            })
                          }
                          style={inp({ width: "100%" })}
                        >
                      ...
                      <div>
                        ...
                        <select
                          value={newTemplate.passing_cefr}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                              passing_cefr: e.target.value,
                            })
                          }
                          style={inp({ width: "100%" })}
                        >
                      ...
                      >
                        {sections.map((s) => (
                          ...
                          >
                            ...
                            <input
                              type="number"
                              value={(newTemplate as any)[`${s}_count`]}
                              onChange={(e) =>
                                setNewTemplate({
                                  ...newTemplate,
                                  [`${s}_count`]: +e.target.value,
                                })
                              }
                              style={{
                                ...inp({
                            ...
                            <input
                              type="number"
                              value={(newTemplate as any)[`weight_${s}`]}
                              onChange={(e) =>
                                setNewTemplate({
                                  ...newTemplate,
                                  [`weight_${s}`]: +e.target.value,
                                })
                              }
                              style={{
                                ...inp({
                    ...
                    >
                      <div>
                        ...
                        <input
                          type="number"
                          value={newTemplate.time_limit_mins}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                              time_limit_mins: +e.target.value,
                            })
                          }
                          style={inp({ width: "100%" })}
                        />
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          value={newTemplate.writing_timer_mins}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                              writing_timer_mins: +e.target.value,
                            })
                          }
                          step={0.5}
                          style={inp({ width: "100%" })}
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          value={newTemplate.speaking_attempts}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                              speaking_attempts: +e.target.value,
                            })
                          }
                          style={inp({ width: "100%" })}
                        />
                    ...
                    <div style={{ display: "flex", gap: "18px" }}>
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={newTemplate.listening_single_play}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                              listening_single_play: e.target.checked,
                            })
                          }
                        />{" "}
                        Single-play audio
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={newTemplate.proctoring_enabled}
                          onChange={(e) =>
                            setNewTemplate({
                              ...newTemplate,
                    ...
                    >
                      ...
                      >
                        Prep Screen Instructions
                      </div>
                      {sections.map((s) => {
                        const prepKey = ("prep_" + s) as any;
                        const prep = (newTemplate as any)[prepKey] || {
                          seconds: 45,
                          bullets: [],
                        };
                        return (
                    ...
                    >
                      <div>
                        ...
                        <select
                          ...
                          style={inp({ width: "100%" })}
                        >
                          {["candidate", "evaluator", "super_admin"].map(
                            (r) => (
                              <option key={r} value={r}>
                      ...
                      <div>
                        ...
                        <select
                          value={formUser.org_id}
                          onChange={(e) =>
                            setFormUser({ ...formUser, org_id: e.target.value })
                          }
                          style={inp({ width: "100%" })}
                        >
                    ...
                    {orgStep === 1 ? (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <input
                            value={formOrg.domain}
                            onChange={(e) =>
                              setFormOrg({ ...formOrg, domain: e.target.value })
                            }
                            placeholder="globalair.com"
                            style={inp({ width: "100%" })}
                          />
                        </div>
                        <div>
                      ...
                      >
                        ...
                        >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              placeholder="jane@globalair.com"
                              style={inp({ width: "100%" })}
                            />
                          </div>
                        </div>
                        <div>
                          ...
                          <input
                            type="date"
                            value={formOrg.contract_end_date}
                            onChange={(e) =>
                              setFormOrg({
                                ...formOrg,
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {orgList.find((o) => o.id === detailUser.org_id)
                            ?.name || "Individual"}
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: "12px", color: "var(--t3)" }}>
                          {detailOrg.domain || "No domain registered"}
                        </div>
                      </div>
                    </div>
                    <div
                      style={{
                        ...
                        borderRadius: "12px",
                        background:
                          new Date(detailOrg.contract_end_date) < new Date()
                            ? "#fef2f2"
                            : "#f0fdf4",
                        border: "1px solid",
                        borderColor:
                    ...
                    >
                      ...
                      >
                        ...
                        <span style={{ fontSize: "13px", color: "var(--t2)" }}>
                          {detailOrg.contract_end_date
                            ? `Ends: ${new Date(detailOrg.contract_end_date).toLocaleDateString()}`
                            : "No date set"}
                        </span>
                      </div>
                    </div>
                    ...
                    {qStep === 1 ? (
                      ...
                      >
                        ...
                        {editQ && (
                          ...
                          >
                            ⚠️ <strong>Auto-Version:</strong> saving will create{" "}
                            <strong>V{(editQ.version_number || 1) + 1}</strong>.
                          </div>
                        )}
                        ...
                        >
                          <div>
                            ...
                            <select
                              value={formQ.section}
                              onChange={(e) =>
                                setFormQ({ ...formQ, section: e.target.value })
                              }
                              style={inp({ width: "100%" })}
                            >
                              {sections.map((s) => (
                          ...
                          <div>
                            ...
                            <select
                              value={formQ.type}
                              onChange={(e) =>
                                setFormQ({ ...formQ, type: e.target.value })
                              }
                              style={inp({ width: "100%" })}
                            >
                              {[
                                ...
                                [
                                  "dla_simulation",
                                  "DLA Simulation (Automatic Phase)",
                                ],
                              ].map((o) => (
                        ...
                        {formQ.section === "dla" && (
                          ...
                          >
                            <div>
                              ...
                              <select
                                value={formQ.dla_section}
                                onChange={(e) =>
                                  setFormQ({
                                    ...formQ,
                                    dla_section: e.target.value,
                                  })
                                }
                                style={inp({ width: "100%" })}
                              >
                                {[
                                  ...
                                  ["scenario", "Emergency Scenarios"],
                                  ["retell", "Retell Passage"],
                                ].map((o) => (
                                  <option key={o[0]} value={o[0]}>
                                    {o[1]}
                                  </option>
                            ...
                            <div>
                              ...
                              <input
                                type="number"
                                value={formQ.answer_time_sec}
                                onChange={(e) =>
                                  setFormQ({
                                    ...formQ,
                                    answer_time_sec: +e.target.value,
                                  })
                                }
                                style={inp({ width: "100%" })}
                              />
                        ...
                        <div>
                          ...
                          <select
                            value={formQ.role_tag}
                            onChange={(e) =>
                              setFormQ({ ...formQ, role_tag: e.target.value })
                            }
                            style={inp({ width: "100%" })}
                          >
                            {[
                      ...
                      >
                        <div>
                          ...
                          <textarea
                            value={formQ.content}
                            onChange={(e) =>
                              setFormQ({ ...formQ, content: e.target.value })
                            }
                            placeholder="..."
                            rows={5}
                            style={{
                        ...
                        <div>
                          ...
                          >
                            Asset (Image/Audio URL)
                          </label>
                          <div style={{ display: "flex", gap: "8px" }}>
                            <input
                              value={formQ.audio_url || formQ.image_url}
                              onChange={(e) =>
                                setFormQ({
                                  ...formQ,
                        ...
                        {formQ.section === "dla" &&
                          formQ.dla_section === "retell" && (
                            <div style={{ marginBottom: "12px" }}>
                              ...
                              >
                                Reading Text (For Retell)
                              </label>
                              <textarea
                                value={formQ.reading_text}
                                onChange={(e) =>
                                  setFormQ({
                                    ...formQ,
                                    reading_text: e.target.value,
                                  })
                                }
                                placeholder="Passage to show candidate for 15 seconds..."
                                rows={4}
                                style={{
                                  ...inp({
                                    width: "100%",
                              ...
                              {departments.map((d) => (
                                <button
                                  ...
                                  style={{
                                    ...
                                    borderRadius: "100px",
                                    border: "1.5px solid",
                                    borderColor: selectedDepts.includes(d.id)
                                      ? "var(--navy)"
                                      : "var(--bdr)",
                                    background: selectedDepts.includes(d.id)
                                      ? "var(--navy)"
                                      : "#fff",
                                    color: selectedDepts.includes(d.id)
                                      ? "#fff"
                                      : "var(--t2)",
                                    fontSize: "11px",
                                    fontWeight: 600,
                                ...
                                {filteredSubRoles.map((s) => (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      borderRadius: "100px",
                                      border: "1.5px solid",
                                      borderColor: selectedSubRoles.includes(
                                        s.id,
                                      )
                                        ? "var(--sky)"
                                        : "var(--bdr)",
                                      background: selectedSubRoles.includes(
                                        s.id,
                                      )
                                        ? "var(--sky3)"
                                        : "#fff",
                                      color: selectedSubRoles.includes(s.id)
                                        ? "var(--sky)"
                                        : "var(--t2)",
                                      fontSize: "11px",
                                      fontWeight: 600,
                              ...
                              {useCases.map((u) => (
                                <button
                                  ...
                                  style={{
                                    ...
                                    borderRadius: "100px",
                                    border: "1.5px solid",
                                    borderColor: selectedUseCases.includes(u.id)
                                      ? "var(--teal)"
                                      : "var(--bdr)",
                                    background: selectedUseCases.includes(u.id)
                                      ? "#E6F7F4"
                                      : "#fff",
                                    color: selectedUseCases.includes(u.id)
                                      ? "var(--teal)"
                                      : "var(--t2)",
                                    fontSize: "11px",
                                    fontWeight: 600,
                  ...
                  >
                    ...
                    >
                      <span
                        style={{
                          padding: "4px 10px",
                          borderRadius: "6px",
                          background: sectionColors[detailQ.section] + "15",
                          color: sectionColors[detailQ.section],
                          fontSize: "11px",
                          fontWeight: 800,
                          textTransform: "uppercase",
                        }}
                    ...
                    {detailQ.audio_url && (
                      ...
                      >
                        ...
                        {detailQ.section === "listening" ? (
                          <audio
                            src={detailQ.audio_url}
                            controls
                            style={{ width: "100%" }}
                          />
                        ) : (
              ...
              {/* end drawer content scroll area */}
              ...
              >
                ...
                {detailUser && !showUserForm && (
                  <div style={{ display: "flex", gap: "10px" }}>
                    ...
                    <button
                      onClick={() => startSingleDeleteUser(detailUser)}
                      style={{
                        padding: "10px 20px",
                        borderRadius: "8px",
                ...
                {detailOrg && !showOrgForm && (
                  <div style={{ display: "flex", gap: "10px" }}>
                    ...
                    <button
                      onClick={() => startSingleDeleteOrg(detailOrg)}
                      style={{
                        padding: "10px 20px",
                        borderRadius: "8px",
                        border: "1.5px solid #FECACA",
