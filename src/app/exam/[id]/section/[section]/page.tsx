...
export default function ExamSectionPage() {
  ...
  async function load() {
    try {
      ...
      const { data: es, error: esErr } = await supabase.from('exam_question_sets').select('*,questions(*)').eq('exam_id', examId).eq('section', section).order('question_order')
      if (esErr) console.error('Question Sets Error:', esErr)
      let qs = es || []
      if (!qs.length) {
        // Role-aware question selection:
        const { data: rolePool, error: rErr } = await supabase.from('questions').select('*')
          .eq('section', section).eq('active', true).eq('is_latest', true)
          .eq('role_tag', roleProfile)
          .limit(cnt * 3)
        if (rErr) console.error('Role Q Error:', rErr)

        const { data: generalPool, error: gErr } = await supabase.from('questions').select('*')
          .eq('section', section).eq('active', true).eq('is_latest', true)
          .eq('role_tag', 'general')
          .limit(cnt * 3)
        if (gErr) console.error('General Q Error:', gErr)

        const roleQs = (rolePool || []).sort(() => Math.random() - 0.5)
        const genQs = (generalPool || []).sort(() => Math.random() - 0.5)
        const roleIds = new Set(roleQs.map((q:any) => q.id))
        const dedupedGeneral = genQs.filter((q:any) => !roleIds.has(q.id))

        const selected = [...roleQs.slice(0, cnt)]
        if (selected.length < cnt) {
          selected.push(...dedupedGeneral.slice(0, cnt - selected.length))
        }
        const sh = selected.sort(() => Math.random() - 0.5).slice(0, cnt)

        if (!sh.length) { setReady(true); return }
        const ins = sh.map((q:any, i:number) => ({ exam_id: examId, question_id: q.id, section, question_order: i }))
        const { error: insErr } = await supabase.from('exam_question_sets').insert(ins)
        if (insErr) console.error('Insert Q Error:', insErr)
        qs = ins.map((x:any, i:number) => ({ ...x, questions: sh[i] }))
      }
      ...
      alert('Sistem Yüklenirken Hata Oluştu / Fatal Load Error:\n' + (err.message || String(err)))
      setReady(true) // escape loading state to show "No questions available" if crash
    }
  }
  ...
  useEffect(() => {
    ...
    if ('mediaSession' in navigator) {
      ...
      navigator.mediaSession.setActionHandler('pause', () => { setTimeout(() => audRef.current?.play().catch(()=>{}), 10) });
    }
  ...
  function playAud() { setLstPhase('play'); audRef.current?.play().catch(()=>{}) }
  ...
  function norm(c: string) { return c.replace(/\\n/g, '\n') }
  ...
  function parseOpts(c: string) {
    ...
    for (const l of lines) {
      ...
      if (m) { opts.push({letter:m[1].toUpperCase(),text:m[2]}); foundOptions = true }
      // Stop adding to question lines if options are found to avoid leakage
      else if (!foundOptions) qLines.push(l)
    }
    let qT = qLines.join('\n').trim()
    
    // Remove any inline options like "A) Option" from text if missed
    const inlineMatch = qT.match(/(?:Option|Seçenek|Opsiyon)?\s*[A-D][.)]\s+.*/i)
    if (inlineMatch && inlineMatch.index !== undefined && inlineMatch.index > 0) {
      qT = qT.substring(0, inlineMatch.index).trim()
    }
    
    qT = qT.replace(/\[AUDIO\]\s*/gi, '')
    return { qT, opts }
  }
  function parseReading(c: string) {
    const raw = norm(c)
    const questionMatch = raw.match(/\n\n([^\n]*\?\s*\n(?:Option|Seçenek|Opsiyon)?\s*[A-D][.)][\s\S]+)/i)
    if (questionMatch) {
      const splitIdx = raw.indexOf(questionMatch[1])
      const pass = raw.substring(0, splitIdx).trim()
      const rest = questionMatch[1].trim()
      const {qT,opts} = parseOpts(rest)
      return {pass, qT: qT || rest, opts}
    }
    const p = raw.split(/\n{2,}/)
      ...
      <div style={{flex:1,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'28px 24px'}}>
        <div style={{width:'100%',maxWidth:'820px'}}>
          ...
          {['grammar','reading','listening'].includes(section) && (() => {
            const p = section==='reading' ? parseReading(cq.content)
                     : section==='listening' ? {pass:'', ...parseListening(cq.content)}
                     : {pass:'', ...parseOpts(cq.content)}
            return <div style={{background:BRAND.white,borderRadius:'16px',padding:'28px',boxShadow:'0 1px 4px rgba(0,0,0,0.06)'}}>
              ...
              {section==='reading' && p.pass && <div style={{background:BRAND.off,borderRadius:'10px',padding:'18px',marginBottom:'14px',border:`1px solid ${BRAND.border}`,fontSize:'14.5px',color:BRAND.text2,lineHeight:1.75,whiteSpace:'pre-wrap'}}>{p.pass}</div>}
              {section==='listening' && cq.audio_url && <div style={{marginBottom:'16px'}}>
                ...
                {lstPhase==='play' && <div style={{background:'#FFF7ED',borderRadius:'10px',padding:'16px',border:'1px solid #FDBA74',textAlign:'center'}}>
                  <div style={{fontSize:'14px',fontWeight:600,color:'#EA580C',marginBottom:'8px'}}>🔊 Audio is playing... Listen carefully.</div>
                  <audio ref={audRef} src={cq.audio_url} controlsList="nodownload noplaybackrate" onContextMenu={e => e.preventDefault()} onEnded={() => setLstPhase('answer')} autoPlay style={{display:'none'}} />
                  <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'6px'}}>
                    <div style={{width:'12px',height:'12px',borderRadius:'50%',background:'#EA580C',animation:'pulse 1.5s infinite'}} />
                    <span style={{fontSize:'13px',color:'#9A3412'}}>Playing — no replay allowed</span>
                  </div>
